...
from multi_Doc_chat.prompts.prompt_liberary import PROMPT_REGISTRY
...
class ConversationalRAG:
    ...
    def _format_docs(docs) -> str:
        ...
        log.info(f"Retrieved {len(docs)} documents, total length: {len(formatted)} chars")
        if len(docs) == 0:
    ...
    def _build_lcel_chain(self):
        try:
            ...
            # Define a function to get context from retriever
            ...
            # Build the chain: Input -> Retrieve Context -> Format Prompt -> LLM -> Output
            self.chain = (
                {"context": RunnableLambda(retrieve_context), "input": itemgetter("input")}
                | self.qa_prompt
                | self.llm
                | StrOutputParser()
            )
